...
int main(void) {
  ...
  for (int i = 0; i <= N; i++) {
    ...
    for (int j = 0; j <= N; j++) {
      y = advar(YMIN + j * hy);
      fprintf(fp, "%f %f ", x.v, y.v);
      adselect(&x, vl);
      z = phi(x, y);
      fprintf(fp, "%f %f ", z.v, -z.dv);
      adselect(&y, vl);
      z = phi(x, y);
      fprintf(fp, "%f\n", -z.dv);
    }
  ...
  fclose(fp);
  plotscript("grad1.dat");
  return 0;
}

adv phi(adv x, adv y) {
  return admul(adconst(Q1), adpow(adadd(adpow(adsub(x, adconst(Q1X)), 2.0),
